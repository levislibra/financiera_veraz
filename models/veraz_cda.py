...
class FinancieraVerazCda(models.Model):
	_name = 'financiera.veraz.cda'

	_order = 'orden asc'
	name = fields.Char('Nombre')
	activo = fields.Boolean('Activo')
	orden = fields.Integer('Orden de ejecucion')
	otorgar_cpm_base = fields.Float('Veraz - CPM Base', digits=(16,2))
	otorgar_cpm_maximo = fields.Float('Veraz - CPM Maximo', digits=(16,2))
	otorgar_partner_tipo_id = fields.Many2one('financiera.partner.tipo', 'Tipo de cliente')
	regla_ids = fields.One2many('financiera.veraz.cda.regla', 'veraz_cda_id', 'Reglas')
	...
	def ejecutar(self, informe_id):
		...
		cda_resultado_id = self.env['financiera.veraz.cda.resultado'].create({
			'name': self.name,
		...
		for regla_id in self.regla_ids:
			...
			regla_resultado_id.veraz_cda_id = None
			regla_resultado_id.veraz_cda_resultado_id = cda_resultado_id.id
...
class FinancieraVerazCdaResultado(models.Model):
	_name = 'financiera.veraz.cda.resultado'

	_order = 'id desc'
	name = fields.Char('Nombre')
	informe_id = fields.Many2one('financiera.veraz.informe', 'Informe')
	regla_ids = fields.One2many('financiera.veraz.cda.regla', 'veraz_cda_resultado_id', 'Reglas')
	otorgar_cpm = fields.Float('Veraz - CPM', digits=(16,2))
	otorgar_cpm_maximo = fields.Float('Veraz - CPM Maximo', digits=(16,2))
	otorgar_partner_tipo_id = fields.Many2one('financiera.partner.tipo', 'Tipo de cliente')
	resultado = fields.Selection([('rechazado', 'Rechazado'), ('aprobado', 'Aprobado')], 'Resultado')

class FinancieraVerazCdaRegla(models.Model):
	_name = 'financiera.veraz.cda.regla'

	veraz_cda_id = fields.Many2one('financiera.veraz.cda', 'CDA Evaluacion')
	veraz_cda_resultado_id = fields.Many2one('financiera.veraz.cda.resultado', 'CDA Resultado')
	...
	def ejecutar(self, informe_id):
		self.ensure_one()
		variable_obj = self.pool.get('financiera.veraz.informe.variable')
		...
		if len(variable_ids) > 0:
			...
			if self.resultado == 'aprobado':
				self.veraz_cda_resultado_id.otorgar_cpm *= self.cpm_multiplicar
				self.veraz_cda_resultado_id.otorgar_cpm += self.cpm_sumar
				if variable_id.valor and variable_id.valor.isdigit():
					self.veraz_cda_resultado_id.otorgar_cpm += int(variable_id.valor) * self.cpm_multiplicar_valor
		else:
